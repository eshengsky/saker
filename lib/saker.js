    ...
    ParseProcessor.prototype = {
        ...
        readMarkup: function () {
            ...
            for (; this.position < len; this.position++) {
                ...
                if (this.readNextChars(4) === 'href') {
                    var a = 1;
                }

                if (char === '<') {
                    //如果是自闭合标签，如 <br>
                    if (/^<(br|hr|img|input|link|meta|area|base|col|command|embed|keygen|param|source|track|wbr)[\s\S]*?>/.test(this.readNextChars())) {

                    }
                    //如果是普通结束标签，如 </div>
                    else if (/^<\/[a-zA-Z0-9]+\s*>/.test(this.readNextChars())) {
                        this.tags.pop();
                        if ((this.tags[this.tags.length - 1] || -1 ) >= (this.braces[this.braces.length - 1] || {position: -1}).position) {
                            result += char;
                            continue;
                        } else {
                            matched = this.readNextChars().match(/^<\/[a-zA-Z0-9]+\s*>/);
                            result += matched[0];
    ...
    var saker = {
        ...
        compile: function (template) {
            ...
            return function (model, cb) {
                ...
                fn += 'this.renderPartial = saker.renderPartial = function(filePath){$saker_data$.push(this.renderPartialFn(filePath, model));};\n';
                        ...
                        if (thisObj.layout) {
                            ...
                            that.getView(thisObj.layout, function (err, layoutTemp) {
                                if (err) {
                                    ...
                                    if (layoutTemp.indexOf('renderBody()') === -1) {
                                        return cb(new Error('layout模板中必须存在renderBody()方法！'));
                                    }
                                    thisObj.layout = null;
